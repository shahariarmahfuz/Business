...
def init_db():
    ...
    # --- MIGRATION: Add updated_at to ledger if not exists ---
    try:
        cur.execute("ALTER TABLE ledger ADD COLUMN IF NOT EXISTS updated_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP")
    except Exception as e:
...
def sales():
    ...
    query += ' ORDER BY s.created_at DESC, s.id DESC LIMIT 100' # Limit for performance
    cur.execute(query, tuple(params))
    sales_data = cur.fetchall()
    cur.execute('SELECT * FROM categories ORDER BY name ASC')
    categories = cur.fetchall()
    cur.close(); conn.close()
    return render_template('sales.html', active_page='sales', sales_data=sales_data, categories=categories, today=datetime.now().strftime('%Y-%m-%d'), edit_item=edit_item)

@app.route('/deposits')
def deposits():
    # ... (Keep existing deposits code) ...
    conn = get_db_connection()
    cur = conn.cursor(cursor_factory=RealDictCursor)
    edit_id = request.args.get('edit_id')
    edit_item = None
    ...
    cur.close(); conn.close()
    return render_template('deposits.html', active_page='deposits', deposits_data=deposits_data, today=datetime.now().strftime('%Y-%m-%d'), edit_item=edit_item)

@app.route('/ledger')
def ledger():
    # ... (Keep existing ledger code) ...
    conn = get_db_connection()
    cur = conn.cursor(cursor_factory=RealDictCursor)
...
def activity():
    ...
    cur.execute('''
        SELECT 'Sale' as type, s.created_at as time, c.name as title, 
               s.total_amount as amount, s.description as details, s.id as id 
        FROM sales s JOIN categories c ON s.category_id = c.id 
        ORDER BY s.created_at DESC LIMIT 30
    ''')
    sales = cur.fetchall()
    
    # 2. Fetch recent Deposits
    cur.execute('''
        SELECT 'Deposit' as type, created_at as time, 'Cash Deposit' as title, 
               amount as amount, note as details, id as id 
        FROM deposits 
        ORDER BY created_at DESC LIMIT 30
    ''')
    deposits = cur.fetchall()
    
    # 3. Fetch recent Ledger Updates (Only where updated_at is set)
    cur.execute('''
        SELECT 'Khata' as type, updated_at as time, ('Page ' || page_no) as title, 
               amount as amount, 'Balance Update' as details, page_no as id 
        FROM ledger 
        WHERE updated_at IS NOT NULL 
        ORDER BY updated_at DESC LIMIT 30
    ''')
    ledger_updates = cur.fetchall()
    
    cur.close()
    conn.close()
    
    # Combine and sort all lists by time
    all_activity = sales + deposits + ledger_updates
    # Sort descending (newest first)
    all_activity.sort(key=lambda x: x['time'] if x['time'] else datetime.min, reverse=True)
    
    return render_template('activity.html', active_page='activity', activities=all_activity)

# --- ACTION ROUTES (Updated for timestamps) ---
...
def add_sale():
    ...
    # Postgres automatically sets created_at
...
def update_sale(id):
    ...
    total = float(f.get('rate')) * float(f.get('quantity'))
    conn = get_db_connection(); cur = conn.cursor()
    # When editing, we might want to update created_at or have a separate updated_at. 
    # For now, we keep created_at as original creation time.
...
# ... (Delete Sale, Add/Update/Delete Deposit remain same) ...
...
def add_category():
    ...
    if name:
        try: conn = get_db_connection(); cur = conn.cursor(); cur.execute('INSERT INTO categories (name) VALUES (%s)', (name,)); conn.commit(); conn.close()
        except: pass 
    return redirect('/categories')
@app.route('/update_category/<int:id>', methods=['POST'])
def update_category(id):
    name = request.form.get('name')
    if name: conn = get_db_connection(); cur = conn.cursor(); cur.execute('UPDATE categories SET name=%s WHERE id=%s', (name, id)); conn.commit(); conn.close()
    return redirect('/categories')
@app.route('/delete_category/<int:id>')
def delete_category(id):
    try: conn = get_db_connection(); cur = conn.cursor(); cur.execute('DELETE FROM categories WHERE id = %s', (id,)); conn.commit(); conn.close()
    except: pass
    return redirect('/categories')

# --- API ENDPOINTS (UPDATED FOR TIMESTAMP) ---

# 1. Bulk Update Ledger (Updated to set updated_at)
...
def bulk_update_ledger():
    try:
        ...
        conn = get_db_connection(); cur = conn.cursor()
        # NOW() will set the current timestamp
        execute_batch(cur, "UPDATE ledger SET amount = %s, updated_at = NOW() WHERE page_no = %s", 
                      [(item['amount'], item['page_no']) for item in updates])
        conn.commit()
...
# 2. Single Ledger Update (Updated to set updated_at)
@app.route('/api/ledger', methods=['POST'])
def api_ledger():
    d = request.json
    conn = get_db_connection(); cur = conn.cursor()
    # Set updated_at = NOW()
    cur.execute('UPDATE ledger SET amount=%s, updated_at=NOW() WHERE page_no=%s', (d['a'], d['p']))
...
def quick_add_sale():
    try:
        ...
        if not cat_id or qty <= 0 or rate <= 0: return jsonify({'status': 'error', 'message': 'Invalid data'}), 400
        total = rate * qty
        conn = get_db_connection(); cur = conn.cursor()
        cur.execute('INSERT INTO sales (category_id, sale_date, description, rate, quantity, total_amount) VALUES (%s, %s, %s, %s, %s, %s) RETURNING id', (cat_id, date, desc, rate, qty, total))
        new_id = cur.fetchone()[0]
        conn.commit(); cur.close(); conn.close()
        return jsonify({'status': 'success', 'id': new_id, 'total_amount': total})
    except Exception as e: return jsonify({'status': 'error', 'message': str(e)}), 500
